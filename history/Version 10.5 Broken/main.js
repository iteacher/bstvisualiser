document.addEventListener('DOMContentLoaded', () => {
            ...
            let initialNodeRadius = 0; // Will be calculated dynamically
            ...
            class BST {
                ...
                assignPosition(node, minX, maxX, currentDepth, maxDepth) {
                    ...
                    const layerHeight = initialNodeRadius * 4; // Vertical spacing multiplier
                    node.y = layerHeight * currentDepth;
            
                    // Calculate horizontal spacing progressively
                    const layerWidth = (maxX - minX) * (currentDepth / maxDepth); // Increase spacing with depth
                    node.x = minX + (maxX - minX) / 2;
            
                    // Assign positions to left and right children
                    const childDepth = currentDepth + 1;
                    const childMaxDepth = maxDepth;
                    const childMinX = node.x - layerWidth / 2;
                    const childMaxX = node.x + layerWidth / 2;
            
                    this.assignPosition(node.left, childMinX, node.x - initialNodeRadius * 2, childDepth, childMaxDepth);
                    this.assignPosition(node.right, node.x + initialNodeRadius * 2, childMaxX, childDepth, childMaxDepth);
                }
                ...
                minValueNode(node) {
                 ...
                 // Method to calculate maximum nodes in any layer
                getMaxNodesPerLayer() {
                    ...
                    while (queue.length > 0) {
                        ...
                        if (level !== currentLevel) {
                            maxNodes = Math.max(maxNodes, currentCount);
                            currentLevel = level;
                            currentCount = 0;
                        }
                    ...
                    maxNodes = Math.max(maxNodes, currentCount); // Check last level
                    return maxNodes;
                }
              ...
              /**
             * Calculates the initial node radius based on container size, tree height,
             * and the maximum number of nodes in any layer.
             * @returns {number} - The calculated node radius in pixels.
             */
              function calculateInitialNodeRadius() {
                const padding = 50; // Padding from the container edges in pixels
                const gap = 10; // Gap between nodes in pixels
            
                const maxNodes = bst.getMaxNodesPerLayer() || 1;
                const treeHeight = bst.getMaxDepth() || 1;
            
                const availableWidth = treeLayer.clientWidth - 2 * padding;
                const availableHeight = treeLayer.clientHeight - 2 * padding;
            
                // Calculate vertical spacing: equal spacing between layers
                const verticalSpacing = availableHeight / (treeHeight);
            
                // Calculate horizontal spacing based on the layer with maximum nodes
                const horizontalSpacing = availableWidth / (maxNodes);
            
                // Calculate node radius based on the smallest spacing dimension
                const radiusWidth = (horizontalSpacing - gap) / 2;
                const radiusHeight = (verticalSpacing - gap) / 2;
            
                // Choose the smaller radius to ensure fit both horizontally and vertically
                return Math.min(radiusWidth, radiusHeight);
            }

            /**
             * Initializes the node radius and updates existing nodes and edges.
             */
            function initializeNodeRadius() {
                initialNodeRadius = calculateInitialNodeRadius();
                // Update existing nodes with the new radius
                nodesGroup.querySelectorAll('.svg-node').forEach(circle => {
                    circle.setAttribute('r', initialNodeRadius);
                });
            
                // Update existing edges if necessary
                updateEdges();
            }
            ...
            function initializeDemo() {
                ...
                // Calculate and set initial node radius after inserting nodes
                bst = new BST(dataType);
                insertNodesintoBST(); // Ensure the BST is populated
                initializeNodeRadius();
                updateTree();
            ...
            function updateNodePositions() {
                if (bst.root !== null) {
                    ...
                    // Assign new positions based on the updated node radius and spacing
                    bst.assignPosition(bst.root, 50, treeLayer.clientWidth - 50, 1, bst.getMaxDepth());
            
                    // Update SVG elements with new positions
                    updateSVGElements();
                }
            }
            ...
            function updateScaleFactor() {
                const scaleFactor = 1.4*window.innerWidth / 2560;
